...
首个 1.0 候选发布版延续 beta 契约，并加入 #130、#132–#134、#147 和 #148 中 beta 之后的 recovery workflow、完整 release gate、精确分发检查、真实 demo 与确定性 core modularization。其规范的 Python/PyPI 版本为 `1.0.0rc1`；面向用户的 Git 标签和 GitHub Release 为 `v1.0.0-rc.1`。`packaging.version.Version` 将这些值视为等同。不要使用裸 `v1.0.0-rc`，它会标准化为 RC 零。

本次发布是明确的预发布版本：

- 将 GitHub Release 标记为 **Pre-release**，切勿标记为 Latest；
- 正常的稳定包升级必须继续选择 v0.9.1；
- 测试人员使用 `pipx install 'session-peer[relay]==1.0.0rc1'` 或等效的 `uv` 命令安装精确版本；
- 在 GitHub、PyPI 以及全新安装验证完成之前，保持 v1.0.0-rc 里程碑处于开启状态；
- 插件清单保留其独立版本（0.1.0）。
...
1. 确认发布分支包含 beta 基线以及 #130、#132–#134、#147、#148、#141，并以 `main` 为目标。
2. 确认 `session_peer.__version__ == "1.0.0rc1"`，候选发布说明已包含在 sdist 中，且四个 README 中的安装命令保持一致。
...
6. 在全新环境中分别独立安装 wheel 和 sdist。确认 `session-peer --version` 报告 `1.0.0rc1`，`session-peer list --output-format
...
2. 确认 PyPI 准确提供了 `session-peer==1.0.0rc1`。下载 wheel 和 sdist，将其哈希值与工作流构件进行比较，并再次检查内容。
...
6. 仅在记录 GitHub、PyPI 及全新安装证据后关闭 v1.0.0-rc 里程碑。
