...
## 当前候选版本：v1.0.0-alpha.1

首个 1.0 alpha 版本依赖于 #99 中的经过身份验证的 relay RC。其规范的 Python/PyPI 版本为 `1.0.0a1`；面向用户的 Git 标签和 GitHub Release 为 `v1.0.0-alpha.1`。`packaging.version.Version` 将这些值视为等同。不要使用裸 `v1.0.0-alpha`，它会标准化为 alpha 零。

本次发布是明确的预发布版本：

- 将 GitHub Release 标记为 **Pre-release**，切勿标记为 Latest；
- 正常的稳定包升级必须继续选择 v0.9.0；
- 测试人员使用 `pipx install 'session-peer[relay]==1.0.0a1'` 或等效的 `uv` 命令安装精确版本；
- 在 GitHub、PyPI 以及全新安装验证完成之前，保持 #101 处于开启状态；
- 插件清单保留其独立版本（0.1.0）。
...
仓库必须在根目录下保留冻结的 `cc_peer.py` 以用于旧版自更新 URL，同时将其从 wheel 和 sdist 中排除。包含所有四个 README、安全策略、alpha 发布说明、中继生命周期/认证文档以及可选的运行时源码。切勿包含 OAuth 凭据、设备密钥、认证数据库、重放状态、浏览器配置文件、本地证据或会话记录。
...
1. 首先合并 #99。将发布准备 PR 的目标重新指向 `main`，对其进行更新，并要求无冲突干净合并。切勿在另一个分支上手动重新创建发布更改。
2. 确认 `session_peer.__version__ == "1.0.0a1"`，alpha 说明已包含在 sdist 中，且四个 README 中的安装命令保持一致。
...
6. 在全新环境中分别独立安装 wheel 和 sdist。确认 `session-peer --version` 报告 `1.0.0a1`，`session-peer list --output-format
...
2. 确认 PyPI 准确提供了 `session-peer==1.0.0a1`。下载 wheel 和 sdist，将其哈希值与工作流构件进行比较，并再次检查内容。
3. 在全新环境中安装确切的 PyPI 预发布版本，并重复版本、空主目录 list、relay 额外依赖项和 MCP 冒烟检查。
4. 确认 GitHub 将该发布标记为预发布且未标记为 latest。稳定的 `releases/latest` 端点和常规更新通知必须继续指向 v0.9.0。
5. 单独验证托管的中继；软件包发布并不能证明服务健康状况、OAuth 策略或代理确认情况。
6. 仅在记录 GitHub、PyPI 及全新安装证据后关闭 #101。

通过包管理器安装的环境使用其自身的管理器进行升级。独立的稳定安装继续使用 `session-peer update`；alpha 测试人员使用精确的软件包版本。提交绝不是已被采用或确认的证明。
