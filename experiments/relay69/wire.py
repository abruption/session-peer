...
def admission(url, credential):
    ...
    req = urllib.request.Request(target, headers={'Authorization': 'Bearer '+credential, 'User-Agent': 'session-peer-relay69-lab/1.0'})
...
async def relay_stream(url, credential):
    ...
    ws = await PinnedConnect(url, additional_headers={'Cookie': cookie}, compression=None,
                       user_agent_header='session-peer-relay69-lab/1.0',
                       max_size=MAX_FRAME, max_queue=4, write_limit=32768,
                       open_timeout=TIMEOUT, close_timeout=2)
    try:
        ready = json.loads(await asyncio.wait_for(ws.recv(), 12))
        ...
        async with asyncio.timeout(TIMEOUT):
            while True:
                try:
                    ...
                    await self.flush()
                    await self.feed()
            if self.ssl.selected_alpn_protocol() != 'session-peer-69-v1':
