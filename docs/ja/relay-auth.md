# プライベートリレーの制御と認証 (v1.0)

ステータス: 実装済みの control-to-relay 契約に関する v1.0 運用者向け文書です。
Abruption のホスト型サービスはパッケージ公開とは別に運用され、この文書は可用性を保証しません。
通常の Python インストール環境にリスナー、OAuth アカウント、Node 依存関係は追加されません。
...
`admin.abruption.dev` は既存の Authelia 管理ポータルとして維持します。
ルートと `/api/*` はすでに認証コンソールが所有しています。正確な
`/session-peer` ポータルページと `/session-peer/api/metrics` の両方を
Authelia で保護してください。集約 API は独立した `127.0.0.1:3771`
リスナーから提供し、公開リレーホスト経由でルーティングしてはいけません。
これにより Authelia が唯一のブラウザログインとなり、公開リレーが転送された
ID ヘッダーを信頼することもありません。リレーへ直接アクセスする場合に備え、
既存の `/admin/metrics` と `/api/admin/metrics` のプロバイダー運用者認可は維持します。
Authelia ポータルでは、ユーザー、公開登録、デバイス、操作について制限された
詳細一覧を取得できます。一覧には名前、メールアドレス、プロバイダー、操作 ID、
状態、世代、短縮された principal が含まれる場合があります。プロバイダーアカウント
ID、内部ユーザー ID、完全な principal、トークン、Cookie、proof、リクエスト hash、
証明書、キーは含めず、各応答は最大 100 行に制限します。
...
コンパイル済みの `node dist/server/migrate.js` を実行する際は、同じ保護された構成と排他的 writer lock を使用し、その後に `npm start` を実行してください。このコマンドは、インストール済みバージョンを報告する前に Better Auth とバージョン管理された session-peer 独自スキーマを移行します。マイグレーションは明示的です。本番起動はそのスキーマを検証し、テーブルを暗黙的に作成または書き換える代わりに `migration_required` で失敗します。アップグレード前に一貫性のある保護スナップショットを取得し、マイグレーションを `ExecStartPre` や自動再起動経路に追加しないでください。ローカル開発では localhost
