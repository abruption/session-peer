...
除 Better Auth 架构外，控制数据库还具有第一方迁移账本。全新安装以及每次涉及架构的升级都必须在启动 HTTP 服务前显式运行已编译的迁移：

```sh
node dist/server/migrate.js
```

请先停止并隔离发布方和中继。创建包含 SQLite 数据库、签名密钥、公有状态和重放高水位状态的受保护一致性快照，然后以控制服务身份、使用其正常的受保护配置和独占 writer lock 运行该命令。迁移具有事务性和幂等性，并保留稳定 principal、代系、吊销 tombstone、操作收据以及单调递增的公有修订版本。

正常启动仅执行验证。旧的、部分的、未来版本的或损坏的架构会在不执行 DDL 的情况下以 `migration_required` 失败。切勿将迁移加入 `ExecStartPre`，删除迁移账本，或在失败后自动恢复旧快照。请保持栈停止，并审查经过隔离的恢复方案。
