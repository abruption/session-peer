...
或者使用 `uv tool install session-peer`。包管理器会安装 `session-peer` 命令，但不会安装 [Claude Code 技能](#the-skill)。要添加该技能以便 Claude 可以自行使用 session-peer：
...
这会将 `session_peer.py` 放置在 `~/.local/share/session-peer/` 中，将 [Claude Code 技能](../../skills/session-peer/SKILL.md) 安装在 `~/.claude/skills/session-peer/` 中，并建立链接 `~/.local/bin/session-peer`。现有的 cc-peer 文件将被保留。使用 `./install.sh --uninstall [--host ...]` 可以将其卸载。
...
独立安装程序将程序放置在 `~/.local/share/session-peer/` 中，将技能单独放置在 `~/.claude/skills/session-peer/SKILL.md` 中。技能放置位置优先遵循 `CLAUDE_CONFIG_DIR`，其次是 `ANTHROPIC_CONFIG_DIR`，最后使用默认路径。该技能引导 Claude 选择目标和消息；Python 程序执行发现和传输。安装它并不会安装 Codex 插件，也不会更改任何一个代理的权限或入站设置。
...
本地使用时请省略 `--host mac`。仅在准备提交时才移除 `--dry-run`。给 `~` 加上引号可使其在目标端进行展开；绝对远程路径同样有效。`--codex-home` 显式选择该副本，但并不是活跃的证据。这种显式指定目录的形式也适用于早于活跃写入者解析功能的版本。
...
如果该 UUID 出现在多个已知目录中，session-peer 会检查每个匹配目录中确切的 `thread-writer-locks/<uuid>.lock`。它会独立探测内核建议锁，并通过两次稳定的 `lsof` 观察来关联打开该文件的进程。只有恰好存在一个稳定的、同一用户的 Codex 写入者时才会选择该目录；在队列提交之前，会立即再次检查该证据。空闲或过期的锁不会仅仅因为其文件存在而胜出。

零个或多个活跃写入者、缺少 `lsof`、权限失败、变动的 PID/inode 以及冲突的证据都会在排队前以安全方式失败（fail-closed）。已归档的保存副本仍然计算在内。无法读取或不兼容的已知数据库以及缺失的已配置数据库也会阻止隐式提交。显式选择 `--codex-home` 可绕过不相关的清单和活动检查。解析后的符号链接别名和重复路径视为同一个目录。

在没有相互竞争的保存目录时，保留原生队列行为。除非 `--codex-home` 精确选择了一个目录，否则 `list` 会汇总已知目录。不存在常规的文件系统扫描或进程环境检查，且进程参数不会被公开。活动检查在目标机器上运行，包括通过 SSH。缺少 POSIX `flock` 或 `lsof` 的平台无法自动解决竞争目录问题，必须使用 `--codex-home`。未配置/自定义的布局以及在检查后创建的副本仍可能被遗漏。使用 `session-peer doctor` 可在不提交的情况下检查所选目录、有界候选列表、可执行文件以及受支持的状态数据库模式。
...
- `codexHomeResolution`：带模式版本的 `status`、`selected`、`reason` 和有界候选证据。状态为 `explicit`、`selected`、`ambiguous` 或 `unknown`；候选者公开已保存线程、写入者锁和稳定所有者 PID 的事实信息，而不包含进程参数或环境变量值。
