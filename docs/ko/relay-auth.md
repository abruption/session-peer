# 프라이빗 릴레이 제어 및 인증 (v1.0)

상태: 구현된 control-to-relay 계약의 v1.0 운영자 참고 문서입니다.
Abruption 호스팅 배포는 패키지 발행과 별도로 운영되며, 이 문서는 서비스 가용성을 보장하지 않습니다.
일반적인 Python 설치 환경에는 리스너, OAuth 계정 또는 Node 의존성을 추가하지 않습니다.
