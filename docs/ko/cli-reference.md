...
사용해도 됩니다. `--codex-home`은 해당 복사본을 명시적으로 선택하지만 활성 상태를
증명하지는 않습니다. 이 명시적 홈 형식은 활성 작성기 해결 기능 이전의 릴리스에서도
작동합니다.
...
UUID가 여러 알려진 홈에 존재하는 경우, session-peer는 일치하는 각 홈의 정확한
`thread-writer-locks/<uuid>.lock`을 검사합니다. 커널 권고 잠금을 독립적으로 조사하고
두 번의 안정적인 `lsof` 관찰을 통해 연 프로세스를 상호 연관시킵니다. 정확히 하나의
안정적인 동일 사용자 Codex 작성기가 해당 홈을 선택하며, 이 증거는 큐 제출 직전에
다시 확인됩니다. 잠금이 비어 있거나 오래된 경우 단지 해당 파일이 존재한다는 이유만으로
선택되지 않습니다.

활성 작성기가 0개이거나 여러 개인 경우, `lsof` 누락, 권한 실패, PID/inode 변경 및
상충되는 증거가 있는 경우 큐에 넣기 전에 안전하게 차단됩니다. 보관된 저장 복사본도
여전히 계산에 포함됩니다. 읽을 수 없거나 호환되지 않는 알려진 데이터베이스 및 누락된
구성 데이터베이스 역시 암시적 제출을 방지합니다. 관련 없는 인벤토리 및 활동 검사를
우회하려면 `--codex-home`을 명시적으로 선택하세요. 확인된 심볼릭 링크 별칭 및 반복된
경로는 하나의 홈으로 계산됩니다.

경쟁하는 저장된 홈이 없는 경우 기본 큐 동작이 유지됩니다. `list`는 `--codex-home`이
정확히 하나를 선택하지 않는 한 알려진 홈들을 집계합니다. 일반적인 파일 시스템 검색이나
프로세스 환경 검사는 없으며, 프로세스 인수는 노출되지 않습니다. 활동 검사는 SSH를
포함하여 대상 머신에서 실행됩니다. POSIX `flock` 또는 `lsof`가 없는 플랫폼은
경쟁 홈을 자동으로 해결할 수 없으므로 `--codex-home`을 사용해야 합니다.
구성되지 않은/사용자 정의 레이아웃 및 검사 후 생성된 복사본은 여전히 누락될 수
있습니다. 제출하지 않고 선택한 홈, 제한된 후보, 실행 파일 및 지원되는 상태 DB 스키마를
검사하려면 `session-peer doctor`를 사용하세요.
...
- `codexHomeResolution`: 스키마 버전 관리형 `status`, `selected`, `reason` 및
  ...
  하나입니다. 후보는 프로세스 인수나 환경 값 없이 저장된 스레드, 작성기 잠금 및
  안정적인 소유자 PID 팩트를 노출합니다.
