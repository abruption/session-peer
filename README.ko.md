...
[기기 페어링·암호화 릴레이](https://github.com/abruption/session-peer/blob/main/docs/ko/paired-devices.md)는 Unix·Python 3.11 이상·`[relay]` 확장이 필요해요. 기기 신원을 고정하며 대상은 운영자가 명시적으로 허용해야 해요. 자체 호스팅 WSS 릴레이는 메시지를 복호화할 수 없으며 NAT 통과나 호스팅된 공용 서비스는 제공하지 않아요.
