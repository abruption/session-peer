...
class StableCandidate(unittest.TestCase):
    def test_package_and_generated_core_version_agree(self):
        self.assertEqual(session_peer.__version__, "1.0.0")
        self.assertIn('__version__ = "1.0.0"',
    ...
    def test_stable_notes_keep_validation_and_limits_visible(self):
        for locale in ("", "ko/", "ja/", "zh-CN/"):
            text = (ROOT / ("docs/" + locale + "releases/v1.0.0.md")).read_text(encoding="utf-8")
            for marker in ("#161", "#162", "#164", "#168", "20/21", "1.0.0",
                           "receiverRoleBusy", "codex", "ACK"):
