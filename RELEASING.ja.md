...
RC3 公開と安定版昇格は別の条件です。公開された正確な RC3 を全 5 ホストで独立 ACK の証拠と中断のない 4 時間観察により検証するまで、#161、#163、#164 と RC3 マイルストーンを開いたままにしてください。[RC3 検証計画](docs/ja/releases/v1.0.0-rc.3.md)に従い、RC2 の証拠やオフライン再現で代替しないでください。

## 現在の候補: v1.0.0-rc.3

3 番目の 1.0 リリース候補は #158 の Windows 修正を含む RC2 契約を維持し、#161 で追跡している公開リレー経路の失敗に対して、送信前の設定再試行を制限したルート診断 (#162) と、受信側の再接続空白の短縮および role_busy 指標 (#168) を追加します。RC2 の Claude と SSH の修正は安定版 v0.9.2 にも公開済みです。正規の Python/PyPI バージョンは `1.0.0rc3` であり、人間向けの Git タグおよび GitHub リリースは `v1.0.0-rc.3` です。`packaging.version.Version` はこれらの値を同一として扱います。RC ゼロに正規化される単なる `v1.0.0-rc` は使用しないでください。

このリリースは明示的なプレリリースです:

- GitHub リリースに **Pre-release** のマークを付け、Latest のマークを付けないでください。
- 通常の安定版パッケージのアップグレードでは、引き続き v0.9.2 が選択される必要があります。
- テスターは `pipx install 'session-peer[relay]==1.0.0rc3'` または同等の `uv` コマンドを使用して正確なバージョンをインストールします。
- GitHub、PyPI、およびクリーンインストールでの検証が完了するまで、v1.0.0-rc.3 マイルストーンをオープンなままにしてください。
...
2. `session_peer.__version__ == "1.0.0rc3"` であること、リリース候補ノートが sdist に含まれていること、および 4 つの README のインストールコマンドが一致していることを確認します。
...
6. クリーンな環境に wheel と sdist を個別にインストールします。`session-peer --version` が `1.0.0rc3` を報告すること、空のホームディレクトリで `session-peer list --output-format
...
2. PyPI が正確に `session-peer==1.0.0rc3` を公開していることを確認します。wheel と sdist をダウンロードし、それらのハッシュをワークフローアーティファクトと比較して、内容を再度検査します。
...
6. GitHub、PyPI、およびクリーンインストールの証拠が記録された後にのみ v1.0.0-rc.3 マイルストーンをクローズしてください。
